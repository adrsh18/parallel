...
float triangleCheck(long pointOneIdx, long pointTwoIdx, long pointThreeIdx);
...
void delaunay(long start, long end, long *leftEdgeIdx, long *rightEdgeIdx)
{
    ...
    if(pointCount == 2) {
        *leftEdgeIdx = *rightEdgeIdx = createEdge(start, end);
            ...
            float direction = triangleCheck(start, start+1, end);
...
static void findLowerTangent(long rightSubLeftEdgeIdx, long s, long leftSubRightEdgeIdx, long u, long *leftLowerEdgeIdx, long *leftLowerOriginIdx, long *rightLowerEdgeIdx, long *rightLowerOriginIdx)
{
    long left, right;
